...
export function Navbar() {
  ...
  const isDarkPage = location.pathname === "/" || location.pathname.startsWith("/projects") || location.pathname === "/about" || location.pathname.startsWith("/about");
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full h-20 border-b z-[100] flex items-center justify-between px-6 md:px-12 transition-all duration-300 backdrop-blur-md ${
      ...
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && (
        <div className={`fixed top-20 left-0 w-full h-[calc(100vh-80px)] flex flex-col items-center justify-center gap-6 lg:hidden z-50 overflow-y-auto animate-in fade-in slide-in-from-top-4 duration-300 ${
          isDarkPage 
            ? "bg-[#050505]/98 text-white" 
            : "bg-[#f5f2ed]/98 text-black"
        }`}>
          {navLinks.map((link) => {
            const isActive = location.pathname === link.path;
            return (
              <Link 
                ...
                className={`w-[85%] max-w-sm text-center py-4 min-h-[48px] flex items-center justify-center font-mono text-xs uppercase font-black tracking-widest transition-all rounded border ${
                  isActive 
                    ? isDarkPage 
                      ? "bg-[#00FF41] text-black border-[#00FF41] shadow-[0_0_20px_rgba(0,255,65,0.4)]" 
                      : "bg-black text-white border-black"
                    : isDarkPage
                      ? "text-white/60 hover:text-[#00FF41] hover:bg-white/5 border-white/5"
                      : "text-black/60 hover:text-accent hover:bg-black/5 border-black/5"
                }`}
              >
                {isActive ? "● " : "/ "} {link.name}
              </Link>
