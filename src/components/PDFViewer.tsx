...
export const PDFViewer: React.FC<PDFViewerProps> = ({ file, title = "Digital Document" }) => {
  ...
  return (
    <div className="relative w-full h-[50vh] sm:h-[60vh] lg:h-[75vh] min-h-[400px] sm:min-h-[500px] lg:min-h-[650px] bg-zinc-950 border-2 border-white/10 rounded-lg overflow-hidden flex flex-col group select-none transition-all duration-300" id="portfolio-pdf-viewer">
      <div className="relative flex-1 w-full h-full bg-zinc-950 flex flex-col justify-center items-center overflow-hidden min-h-0" id="pdf-frame-area">
        ...
        {isLoading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-950/95 z-20">
            ...
            <span className="font-mono text-[9px] uppercase tracking-widest text-zinc-400">Loading document...</span>
          </div>
        )}
